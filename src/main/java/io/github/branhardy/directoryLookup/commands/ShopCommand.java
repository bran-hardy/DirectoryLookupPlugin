    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        DirectoryLookup.instance
                ...
                .runTaskAsynchronously(DirectoryLookup.instance, () -> {
                    ...
                    // If no shops are found, let the player know
                    if (shops.isEmpty()) {
                    ...
                    if (filter.size() > 1 && !shops.isEmpty()) {
                        TextComponent filterMessage = new TextComponent("Note: The " + shopSuffix + " may not sell the exact item, but have similar items stock");

                        filterMessage.setColor(net.md_5.bungee.api.ChatColor.GRAY);
                        filterMessage.setItalic(true);

                        sender.spigot().sendMessage(filterMessage);
                    }

                    // Send all the shop info to the player
                    for (Shop shop : shops) {
